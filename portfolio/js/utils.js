...
/**
 * Lightweight SplitText — splits element text into per-char <span>s.
 * Returns { chars, words, revert }
 */
class SplitText {
  constructor(el, opts = {}) {
    this.el = typeof el === 'string' ? document.querySelector(el) : el;
    if (!this.el) return;
    this.opts = { type: 'chars', lineClass: 'split-line', wordClass: 'split-word', charClass: 'split-char', ...opts };
  ...
  _run() {
    ...
    wordArr.forEach(token => {
      ...
      if (type === 'chars' || type === 'chars,words' || type === 'words,chars') {
        [...token].forEach(ch => {
  ...
  revert() {
    if (this._original !== undefined) this.el.innerHTML = this._original;
  }
}
